...
class ImageGraphicsView(QGraphicsView):
        ...
        if self.is_rotating and self.last_mouse_pos:
            ...
            if self.fixed_y_mode and not self.fixed_y_line_fixed:
                ...
                if not self.floating_annotation_item:
                    ...
                    self.floating_annotation_item.setRotation(
                        -self.rotation_angle)
    ...
    def rotate_image(self, angle):
        if self.image_group:
            ...
            group_bounds = self.image_group.mapToScene(
                self.image_group.boundingRect()).boundingRect()
            self.scene.setSceneRect(
                group_bounds.adjusted(-10, -10, 10, 10))
    ...
    '''def save_image(self, save_path):
        ...
        rect = self.image_group.mapToScene(
            self.image_group.boundingRect()).boundingRect().adjusted(
            -10, -10, 10, 10)
